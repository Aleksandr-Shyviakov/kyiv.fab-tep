...
if (!class_exists('Kuteshop_Welcome')) {
    class Kuteshop_Welcome
    {
        ...
        {
            $this->theme_name = wp_get_theme(get_template())->get('Name');
            add_action('init', array($this, 'set_tabs'), 9);
        ...
        public function dashboard_admin_scripts($hook)
        {
            if ($hook == 'toplevel_page_kuteshop_menu') {
                ...
                wp_enqueue_script(
                    'kuteshop-dashboard',
                    KUTESHOP_TOOLKIT_URL.'assets/dashboard.js', array(),
                    KUTESHOP_VERSION
                );
            }
            if ($hook == 'index.php') {
        ...
        {
            $this->tabs['dashboard'] = esc_html__('Welcome', 'kuteshop-toolkit');
            if (apply_filters('ovic_disable_envato_license', false) == false) {
                include_once dirname(__FILE__).'/envato.php';
                $this->tabs['license'] = esc_html__('Envato License', 'kuteshop-toolkit');
            }
            $this->tabs['more_theme'] = esc_html__('More Theme', 'kuteshop-toolkit');
            $this->tabs['plugins']    = esc_html__('Plugins', 'kuteshop-toolkit');
            $this->tabs['changelog']  = esc_html__('ChangeLog', 'kuteshop-toolkit');
        }
