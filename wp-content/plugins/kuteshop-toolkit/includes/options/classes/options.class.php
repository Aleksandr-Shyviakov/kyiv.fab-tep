...
if (!class_exists('OVIC_Options')) {
    class OVIC_Options extends OVIC_Abstract
    {
        ...
        {
            $this->args    = apply_filters("ovic_framework_{$this->unique}_settings", wp_parse_args($args, $this->args),
                $this);
            ...
            add_action('admin_bar_menu',
                array(&$this, 'add_admin_bar_menu'), $this->args['admin_bar_menu_priority']
            );
        ...
        public function set_options($ajax = false)
        {
            ...
            $response = ($ajax && !empty($_POST['data'])) ? json_decode(wp_unslash(trim($_POST['data'])),
                true) : $_POST;
        ...
        public function add_options_html()
        {
            ...
            echo '<input type="submit" name="'.esc_attr($this->unique).'[_nonce][save]" class="button button-primary ovic-top-save ovic-save'.esc_attr($ajax_class).'" value="'.esc_html__('Save',
            ...
            if (!empty($this->args['show_footer'])) {
                ...
                echo '<div class="ovic-buttons">';
                echo '<input type="submit" name="ovic_transient[save]" class="button button-primary ovic-save'.esc_attr($ajax_class).'" value="'.esc_html__('Save',
                        'ovic-addon-toolkit').'" data-save="'.esc_html__('Saving...', 'ovic-addon-toolkit').'">';
                echo ($this->args['show_reset']) ? '<input type="submit" name="ovic_transient[reset_section]" class="button button-secondary ovic-reset-section ovic-confirm" value="'.esc_html__('Reset Section',
                        'ovic-addon-toolkit').'" data-confirm="'.esc_html__('Are you sure to reset this section options?',
                        'ovic-addon-toolkit').'">' : '';
