...
if (!class_exists('OVIC_Field_map')) {
    class OVIC_Field_map extends OVIC_Fields
    {
        ...
        public function render()
        {
            ...
            wp_enqueue_script('ovic-leaflet');
            wp_enqueue_style('ovic-leaflet');
            ...
            if (empty($args['address_field'])) {
                ...
                echo '<div class="ovic--address-field" data-address-field="'.esc_attr($args['address_field']).'"></div>';
            }

            echo '<div class="ovic--map-osm-wrap"><div class="ovic--map-osm" data-map="'.esc_attr(json_encode($settings)).'"'.$style_attr.'></div></div>';

            echo '<div class="ovic--map-inputs">';

            echo '<div class="ovic--map-input">';
            echo '<label>'.esc_attr($args['latitude_text']).'</label>';
            echo '<input type="text" name="'.esc_attr($this->field_name('[latitude]')).'" value="'.esc_attr($value['latitude']).'" class="ovic--latitude" />';
            echo '</div>';

            echo '<div class="ovic--map-input">';
            echo '<label>'.esc_attr($args['longitude_text']).'</label>';
            echo '<input type="text" name="'.esc_attr($this->field_name('[longitude]')).'" value="'.esc_attr($value['longitude']).'" class="ovic--longitude" />';
            echo '</div>';

            echo '</div>';

            echo '<input type="hidden" name="'.esc_attr($this->field_name('[zoom]')).'" value="'.esc_attr($value['zoom']).'" class="ovic--zoom" />';
        ...
        public function enqueue()
        {
            ...
            if (!wp_script_is('ovic-leaflet')) {
                wp_register_script('ovic-leaflet', esc_url($this->cdn_url.$this->version.'/dist/leaflet.min.js'), array('ovic-options'), $this->version, true);
            }

            if (!wp_style_is('ovic-leaflet')) {
                wp_register_style('ovic-leaflet', esc_url($this->cdn_url.$this->version.'/dist/leaflet.min.css'), array(), $this->version);
            }
