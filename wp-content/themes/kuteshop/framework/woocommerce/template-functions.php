...
if (class_exists('YITH_WCQV_Frontend')) {
    ...
    if ((!wp_is_mobile() && $enable) || (wp_is_mobile() && $enable_on_mobile && $enable)) {
        add_action( 'init', function () use ( $class_quick_view ) {
            remove_action( 'woocommerce_after_shop_loop_item', array(
                $class_quick_view,
                'yith_add_quick_view_button'
            ), 15 );
        } );
        add_action('kuteshop_function_shop_loop_item_quickview', array(
            $class_quick_view,
            'yith_add_quick_view_button'
        ), 5);
    }
...
if (!function_exists('kuteshop_template_loop_product_title')) {
    function kuteshop_template_loop_product_title()
    {
        ...
        <h3 class="product-name product_title">
            <a href="<?php echo esc_url($permalink); ?>"><?php echo get_the_title(); ?></a>
        </h3>
        <?php
    }
}
...
if (!function_exists('kuteshop_process_availability')) {
    function kuteshop_process_availability()
    {
        global $product;
        ...
        <div class="process-availability">
            <div class="availability-text">
					...
					<?php
                    $stock = esc_html__('Unlimit', 'kuteshop');
                    if (!empty($availability) && $availability > 0) {
                        $stock = $availability;
                    }
                    echo sprintf('%s <strong>%s</strong>', esc_html__('Available:', 'kuteshop'), $stock);
                    ?>
					</span>
                <span class="text sold"><?php echo sprintf('%s <strong>%s</strong>', esc_html__('Already sold:', 'kuteshop'), $units_sold); ?></span>
            </div>
