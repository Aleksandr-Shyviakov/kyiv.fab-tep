...
//			add_action('init', array( $this, '_register' ), 10);
		...
		{
			$labels = array(
				'name'          => esc_html__( 'Mega Menu', 'kuteshop' ),
				'singular_name' => esc_html__( 'Mega Menu', 'kuteshop' ),
				'all_items'     => esc_html__( 'Mega Menu', 'kuteshop' ),
			);
			$args   = array(
				'labels'              => $labels,
				'public'              => true,
		...
		public static function install_mobile_menu(
			$menu_locations,
			$default = 'primary',
		...
		public function _clone_mobile_menu()
		{
			...
			if ( ! empty( $menus ) ) {
				...
				echo apply_filters( 'ovic_menu_toggle_mobile',
					'<button class="menu-toggle"><span>' . esc_html__( 'Menu Mobile',
						'kuteshop' ) . '</span></button>' );
			}
		}
		...
		public static function _return_title_menu_item( $menu_item, $settings )
		{
			...
			if ( $menu_icon_type == 'font-icon' ) {
				...
				if ( isset( $settings['icon_image'] ) && $settings['icon_image'] != '' && $settings['icon_image'] > 0 ) {
					$image = wp_get_attachment_image( $settings['icon_image'], 'full', false,
			...
			if ( isset( $settings['label_image'] ) && $settings['label_image'] != '' && $settings['label_image'] > 0 ) {
				$image = wp_get_attachment_image( $settings['label_image'], 'full', false,
		...
		public static function _return_megamenu_item( $menu_item, $settings, $depth = 0 )
		{
			...
			if ( $enable_mega == true ) {
				...
				if ( $menu_content_id > 0 && $enable_mega == true ) {
					...
					if ( isset( $settings['menu_bg'] ) && $settings['menu_bg'] > 0 && $image = wp_get_attachment_image_url( $settings['menu_bg'],
							'full' ) ) {
					...
					if ( self::is_elementor( $menu_content_id ) ) {
						$_output .= \Elementor\Plugin::$instance->frontend->get_builder_content_for_display( $menu_content_id, true );
					} else {
