...
if ( ! function_exists( 'kuteshop_get_logo' ) ) {
	function kuteshop_get_logo()
	{
		...
		$logo      = kuteshop_get_option_meta( '_custom_metabox_theme_options', 'kuteshop_logo', 'metabox_kuteshop_logo' );
		if ( $logo != '' ) {
...
if ( ! function_exists( 'kuteshop_get_header' ) ) {
	...
	{
		$header = kuteshop_get_option_meta( '_custom_metabox_theme_options',
...
if ( ! function_exists( 'kuteshop_get_footer' ) ) {
	...
	{
		$footer_options        = kuteshop_get_option_meta( '_custom_metabox_theme_options',
			'kuteshop_footer_options',
			'kuteshop_metabox_footer_options',
			'style-01'
		);
		...
		if ( $query->have_posts() ):
			while ( $query->have_posts() ): $query->the_post();
				...
				if ( class_exists( '\Elementor\Plugin' ) && in_array( $post_type, $cpt_support ) ) {
					echo \Elementor\Plugin::$instance->frontend->get_builder_content_for_display( get_the_ID(), true );
				} else {
...
if ( ! function_exists( 'kuteshop_get_header_menu' ) ) {
	function kuteshop_get_header_menu( $args )
	{
		if ( ! empty( $args['menu'] ) && has_nav_menu( $args['menu'] ) ) {
			if ( $args['menu'] == 'primary' ) {
				$primary_menu = kuteshop_get_meta( '_custom_metabox_theme_options', 'primary_menu' );
				if ( ! empty( $primary_menu ) ) {
					$args['menu'] = $primary_menu;
				}
			}
			wp_nav_menu( $args );
		}
