    ...
    class Breadcrumb_Trail
    {
        ...
        public function trail()
        {
            ...
            if (0 < $item_count) {
                ...
                $breadcrumb .= '<ul class="trail-items breadcrumb" itemtype="'.esc_url('http://schema.org/BreadcrumbList').'">';
                ...
                foreach ($this->items as $item) {
                    ...
                    $item = !empty($matches) ? sprintf('%s<span itemprop="name">%s</span>%s', $matches[1], $matches[2],
                        $matches[3]) : sprintf('<span itemprop="name">%s</span>', $item);
                    ...
                    $attributes = 'itemprop="itemListElement" itemtype="'.esc_url('http://schema.org/ListItem').'" class="'.$item_class.'"';
        ...
        protected function add_term_archive_items()
        {
            ...
            if (false === $done_post_type && 1 === count($taxonomy->object_type) && post_type_exists($taxonomy->object_type[0])) {
                ...
                if ('post' === $taxonomy->object_type[0]) {
                    ...
                    if ('posts' !== get_option('show_on_front') && 0 < $post_id) {
                        $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_permalink($post_id)),
            ...
            if (is_paged()) {
                $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_term_link($term, $term->taxonomy)),
        ...
        protected function add_user_archive_items()
        {
            ...
            if (is_paged()) {
                $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_author_posts_url($user_id)),
        ...
        protected function add_minute_hour_archive_items()
        {
            ...
            if (true === $this->args['show_title']) {
                $this->items[] = sprintf($this->labels['archive_minute_hour'],
                    get_the_time(esc_html_x('g:i a', 'minute and hour archives time format', 'kuteshop')));
            }
        }
        ...
        protected function add_minute_archive_items()
        {
            ...
            if (true === $this->args['show_title']) {
                $this->items[] = sprintf($this->labels['archive_minute'],
                    get_the_time(esc_html_x('i', 'minute archives time format', 'kuteshop')));
            }
        }
        ...
        protected function add_hour_archive_items()
        {
            ...
            if (true === $this->args['show_title']) {
                $this->items[] = sprintf($this->labels['archive_hour'],
                    get_the_time(esc_html_x('g a', 'hour archives time format', 'kuteshop')));
            }
        }
        ...
        protected function add_day_archive_items()
        {
            ...
            $year  = sprintf($this->labels['archive_year'],
                get_the_time(esc_html_x('Y', 'yearly archives date format', 'kuteshop')));
            $month = sprintf($this->labels['archive_month'],
                get_the_time(esc_html_x('F', 'monthly archives date format', 'kuteshop')));
            $day   = sprintf($this->labels['archive_day'],
                get_the_time(esc_html_x('j', 'daily archives date format', 'kuteshop')));
        ...
        protected function add_week_archive_items()
        {
            ...
            $week = sprintf($this->labels['archive_week'],
                get_the_time(esc_html_x('W', 'weekly archives date format', 'kuteshop')));
            // Add the year item.
            $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_year_link(get_the_time('Y'))), $year);
            // Add the week item.
            if (is_paged()) {
        ...
        {
            if (is_paged()) {
                $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_search_link()),
        ...
        protected function add_post_hierarchy(
            ...
            if ($post_type_object->has_archive) {
                ...
                // Core filter hook.
                $label         = apply_filters('post_type_archive_title', $label, $post_type_object->name);
                $this->items[] = sprintf('<a href="%s">%s</a>', esc_url(get_post_type_archive_link($post_type)),
                    $label);
            }
        }
