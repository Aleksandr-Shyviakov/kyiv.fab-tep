(function ( $ ) {
    ...
    $.fn.kuteshop_countdown              = function () {
        ...
        $( this ).each( function () {
            ...
            $this.countdown( $this.data( 'datetime' ), { defer: false } ).on( 'update.countdown', function ( event ) {
    ...
    $( document ).on( 'click', '.view-all-category > a', function () {
        var button   = $( this ),
            category = button.closest( '.block-nav-category' ),
            items    = category.data( 'items' ),
            open     = button.data( 'alltext' ),
            close    = button.data( 'closetext' ),
            list     = category.find( 'ul.vertical-menu > li:nth-child(n+' + (items + 1) + ')' );
    ...
    $.fn.kuteshop_add_notify = function ( $text_content ) {
        ...
        if ( typeof $title === 'undefined' || $title === '' ) {
            $title = $this.closest( '.product-item' ).find( '.product_title a' ).text().trim();
        }
        ...
        if ( $img.length ) {
            $img_html = '<figure><img src="' + $img.attr( 'src' ) + '"' + ' alt="' + $title + '" class="growl-thumb" /></figure>';
        }

        config.message = $img_html + '<p class="growl-content">' + $title + '' + $text_content + '</p>';
    ...
    $( document ).on( 'added_to_wishlist removed_from_wishlist', function ( event, $button ) {
        ...
        var html       = '',
            ...
            message    = kuteshop_params.growl_notice.added_to_wishlist_text;

        if ( is_remove === true ) {
            message = kuteshop_params.growl_notice.removed_from_wishlist_text;
        }

        html += message + '</br>';
        html += '<a href="' + kuteshop_params.growl_notice.wishlist_url + '">';
        if ( is_remove === false ) {
            html += kuteshop_params.growl_notice.browse_wishlist_text;
        }
        html += '</a>';

        target.kuteshop_add_notify( html );
    ...
    $( window ).on( 'wc_fragments_refreshed wc_fragments_loaded', function () {
